...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ОбъектыНазначения) <> Тип("Массив") Тогда 
		...
		Если НЕ ЗначениеЗаполнено(Параметры.ИдентификаторКоманды) Тогда 
			Параметры.ИдентификаторКоманды = "ЗаполнитьПоФайлуXML";
		КонецЕсли;
	Иначе	
	...
	Сообщить("При создании на сервере Параметры.ДополнительнаяОбработкаСсылка = " + Параметры.ДополнительнаяОбработкаСсылка);
...
&НаКлиенте
Процедура ЗаполнитьПоФайлуXML(Команда)
	...
	сообщить("начато помещение файла");
...
Процедура ОповещениеПомещениеФайла(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	сообщить("закончено помещение файла");
	...
	сообщить("закончено помещение файла - продолжаем 1 " + Параметры.ДополнительнаяОбработкаСсылка);
	...
	Если ЗначениеЗаполнено(Параметры.ДополнительнаяОбработкаСсылка) Тогда
		сообщить("закончено помещение файла - продолжаем 2 ");
		ДополнительныеОтчетыИОбработкиКлиент.ВыполнитьКомандуВФоне(Параметры.ИдентификаторКоманды, ПараметрыКоманды, Обработчик);
		сообщить("закончено помещение файла - продолжаем 3 ");
	Иначе
		сообщить("закончено помещение файла - продолжаем 4 ");
		...
		сообщить("закончено помещение файла - продолжаем 5 ");
		ВыполнитьОбработкуОповещения(Обработчик, Операция);
		сообщить("закончено помещение файла - продолжаем 6 ");
	КонецЕсли;
