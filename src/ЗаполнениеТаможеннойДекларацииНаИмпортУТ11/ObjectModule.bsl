...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Информация = НСтр("ru = 'Обработка заполнения документа ""Таможенная декларация на импорт"" по файлу XML жестко заданной структуры (вер.1.4)""'");
...
Процедура ЗаполнитьПоФайлуXML(ОбъектыНазначения, ПараметрыВыполнения) Экспорт
	...
	ПоказыватьТехническиеСообщения = РольДоступна("ПолныеПрава");
			...
			Иначе
				...
				Для Каждого иСтрокаТовара Из иДокумент.Товары Цикл
					...
					Если ЗначениеЗаполнено(иСтрокаТовара.Артикул) Тогда 
						...
						мсвНайденоПоАртикулу = тзНоменклатура.Скопировать(тзНоменклатура.НайтиСтроки(Новый Структура("Артикул", иСтрокаТовара.Артикул))).ВыгрузитьКолонку("Ссылка"); 
					Иначе
					...
					Если ПоказыватьТехническиеСообщения И ИскомоеКоличество > 0 Тогда
						...
						Сообщение.Текст 		= 
							СтрШаблон(
							НСтр("ru = 'В 1С не найдена позиция из файла XML: 
							| 	Наименование: %1
							| 	Артикул: %2
							| 	Производитель: %3
							| 	Упаковка: %4
							...
							| 	НомерРаздела: %6'"),
							Строка(иСтрокаТовара.Номенклатура),
							Строка(иСтрокаТовара.Артикул),
							Строка(иСтрокаТовара.Производитель),
							Строка(иСтрокаТовара.Упаковка),
							Строка(иСтрокаТовара.КоличествоУпаковок),
							Строка(иСтрокаТовара.НомерРаздела)
							);
						Сообщение.Сообщить();
					КонецЕсли;
				КонецЦикла;
