	...
	Для Каждого ЭлементОбъектНазначения Из ОбъектыНазначения Цикл
		...
		Для Каждого иДокумент Из ДанныеXML Цикл
			...
			Для Каждого иРаздел Из иДокумент.Разделы Цикл
				...
				НоваяСтрокаРазделов.СтавкаНДС 			= НайтиСтавкуНДСПоЧислу(иРаздел.СтавкаНДС);
				НоваяСтрокаРазделов.СтавкаПошлины 		= иРаздел.СтавкаПошлины;
				НоваяСтрокаРазделов.СтранаПроисхождения = НайтиСтрануМираПоНаименованию(иРаздел.СтранаПроисхождения);
			...
			Иначе
				...
				Для Каждого иСтрокаТЧТовары Из ОбъектНазначения.Товары Цикл
					...
					ПереборомНайдем
					//СтрокаТовараСтруктура.КоличествоУпаковок 	= Число(иСтрокаТовара.GoodsGroupQuantity.GoodsQuantity);
					...
					//СтрокаТовараСтруктура.Упаковка 				= иСтрокаТовара.GoodsGroupQuantity.MeasureUnitQualifierCode; // лучше код, а не имя, как в MeasureUnitQualifierName;
					//СтрокаТовараСтруктура.Производитель 		= иСтрокаТовара.TradeMark;
					//СтрокаТовараСтруктура.Артикул 				= иСтрокаТовара.GoodsModel;
	...
	Для Каждого иСтрокаРаздела Из СписокРазделыГТД Цикл
		...
		Для Каждого иСтрокаПошлиныИСборы Из СписокПошлиныИСборыРаздела Цикл
			Если иСтрокаПошлиныИСборы.PaymentModeCode = КодПошлина Тогда 
				...
				СтавкаПошлиныРаздела 	= Число(иСтрокаПошлиныИСборы.Rate);
			ИначеЕсли иСтрокаПошлиныИСборы.PaymentModeCode = КодНДС Тогда 
				...
				СтавкаНДСРаздела 		= Число(иСтрокаПошлиныИСборы.Rate);
			КонецЕсли;
		КонецЦикла;
