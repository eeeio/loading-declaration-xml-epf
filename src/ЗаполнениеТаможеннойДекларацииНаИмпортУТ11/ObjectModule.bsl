...
Процедура ЗаполнитьПоФайлуXML(ОбъектыНазначения, ПараметрыВыполнения) Экспорт
	...
	ПутьКФайлу = ПараметрыВыполнения.ПутьКФайлуНаСервере;
	
	НайденныеДанныеXML = Новый Массив;
	...
	Для Каждого иЗаполнено Из НайденныеДанныеXML Цикл
		ЗаголовокОповещения = ЗаголовокОповещения + Символы.ПС + иЗаполнено;
	КонецЦикла;
