...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Информация = НСтр("ru = 'Обработка заполнения документа ""Таможенная декларация на импорт"" по файлу XML жестко заданной структуры (вер.1.0)""'");
		...
		Для Каждого иДокумент Из ДанныеXML Цикл
			...
			Иначе
				...
				Для Каждого иСтрокаТовара Из иДокумент.Товары Цикл
					...
					Если СтрокиДокументаПоТовару[0].НомерРаздела = 0 Тогда 
...
Функция НайтиПохожиеПоНаименованию(пНаименование, тзНоменклатура) 
	...
	МассивСлов = СтрРазделить(пНаименование, " ", Ложь);
    ...
    Для Каждого иНом Из тзНоменклатура Цикл
		ПроцентСовпадения = ПолучитьПроцентСходстваНаименований_2(иНом.Наименование, МассивСлов, ОбщДлинаСлов);
